...
function normalizeState(data) {
  ...
  return {
    books: (data.books || demoBooks).map((book) => ({
      ...book,
...
function renderCatalog() {
  ...
  books.forEach((book) => {
    ...
    card.innerHTML = `
      <div class="book-cover ${heroImage ? "has-image" : ""}" data-cover-for="${book.id}" ${coverStyle}>
        <span>${escapeHtml(book.title)}</span>
      </div>
      <div class="book-body">
        <div>
          <h3>${escapeHtml(book.title)}</h3>
          <p class="meta">${escapeHtml(book.author)} · ${escapeHtml(book.category || "General")}</p>
        </div>
        <p class="description">${escapeHtml(book.description || "No description yet.")}</p>
        <div class="product-actions">
          <select data-variant-for="${book.id}" data-book-cover="${escapeAttr(book.cover || "")}" aria-label="Choose variation for ${escapeAttr(book.title)}">
            ${book.variants.map((variant) => `
              <option value="${variant.id}" data-photo="${escapeAttr(variant.photo || "")}" ${firstAvailable?.id === variant.id ? "selected" : ""} ${variant.stock <= 0 ? "disabled" : ""}>
                ${escapeHtml(variant.label)} - ${formatPrice(variant.price)} (${variant.stock} left)
              </option>
            `).join("")}
          </select>
          ${firstAvailable?.photo ? `<img class="variation-photo" data-variant-preview-for="${book.id}" src="${escapeAttr(firstAvailable.photo)}" alt="${escapeAttr(firstAvailable.label)}">` : `<span class="pill" data-variant-preview-for="${book.id}">Photo</span>`}
          <input type="number" min="1" value="1" data-qty-for="${book.id}" aria-label="Quantity for ${escapeAttr(book.title)}">
          <button class="primary-button" type="button" data-add="${book.id}" ${!firstAvailable || firstAvailable.stock <= 0 ? "disabled" : ""}>Add to cart</button>
        </div>
      </div>
    `;
...
function addToCart(bookId) {
  ...
  const variant = getVariant(book, variantSelect.value);
...
els.logoutAdminBtn.addEventListener("click", () => switchMode("customer"));
